...
function LeaseOptionCard({ option }: { option: string }) {
  ...
  return (
    <div className="flex h-[100px] sm:h-[120px] md:h-auto md:min-h-[120px] w-full flex-col items-center justify-center rounded-lg p-3 sm:p-4 md:p-6 text-center border border-blue-400/80 bg-gray-50 font-light">
      <div className="mb-1 text-brand-muted text-xs sm:text-sm md:text-base truncate sm:truncate md:text-clip w-full">{duration}</div>
      <div className="text-sm sm:text-base md:text-xl mb-1 text-brand-accent font-medium truncate sm:truncate md:text-clip w-full px-1">
        GH₵ {price}
      </div>
      <div className="text-brand-muted text-xs sm:text-sm md:text-base truncate sm:truncate md:text-clip w-full">per {unit}</div>
    </div>
  );
}
...
export default function LeaseOptions({ leaseOptions }: LeaseOptionsProps) {
  ...
  return (
    <aside className="pt-14 md:pt-20">
      <Card className="flex w-full flex-col items-start gap-4 sm:gap-5 rounded-lg border-[#50a3ff] bg-[#d7e9ff] px-4 sm:px-6 py-4 text-brand-accent md:flex-row">
        <Badge
          id="lease-options-heading"
          className="bg-blue-500 text-xs uppercase shrink-0"
        >
          Available Lease options
        </Badge>
        <ul
          className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 w-full gap-3 sm:gap-4 md:gap-3 min-w-0"
          role="list"
