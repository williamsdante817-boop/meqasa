...
export function detectReferenceType(input: string): ReferenceDetectionResult {
  ...
  if (/^\d+$/.test(clean)) {
    return {
      ...
      reference: originalInput,
      reasoning:
        "Generic numeric - slight preference for established property system",
    };
  }
  ...
  if (/[A-Z]/.test(clean) && /\d/.test(clean)) {
    return {
      ...
      reference: originalInput,
      reasoning:
        "Mixed alphanumeric - units more likely to have complex patterns",
    };
  }
...
export function determineSearchStrategy(
  ...
  return {
    strategy: "parallel",
    explanation:
      "Low confidence in reference type - searching both properties and units simultaneously",
  };
}
 ...
 */
export function enhancedReferenceDetection(
  input: string
 ...
 */
export function smartReferenceClean(
  reference: string,
  type?: ReferenceType
): string {
