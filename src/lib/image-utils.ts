...
export const getCDNBaseUrl = (): string => {
  return process.env.NEXT_PUBLIC_CDN_URL || 'https://dve7rykno93gs.cloudfront.net';
};
...
export const sanitizeImageArray = (images: (string | undefined | null)[]): string[] => {
  return images
...
class ImagePreloader {
  ...
  async preloadImage(url: string, options: { priority?: boolean } = {}): Promise<void> {
...
export const preloadImageOptimized = (url: string, options?: { priority?: boolean }) => 
