 ...
 */
export async function generateUnitUrlFromReference(unitReference: string): Promise<string> {
 ...
 */
export async function processUnitReferenceSearch(reference: string): Promise<UnitReferenceSearchResult> {
  ...
  try {
    ...
    try {
      ...
      const genericSlug = generateGenericUnitSlug(cleanRef).replace('/developer-unit/', '');
      ...
      const isUnitNotFound = errorMessage.includes("not available") || 
      ...
      if (isUnitNotFound) {
        return {
          ...
          source: 'api',
          responseTime: performance.now() - startTime,
          error: "Unit not found. Please check the reference number and try again."
        };
      }
      ...
      return {
        ...
        source: 'fallback',
        responseTime: performance.now() - startTime,
        error: "Network issue - using approximate URL. Full details will load on page."
      };
    }
 ...
 */
export function processUnitReferenceSearchFallback(reference: string): UnitReferenceSearchResult {
  const startTime = performance.now();
