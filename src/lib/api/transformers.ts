 ...
 */
export function transformApiToPropertyListing(apiData: unknown): PropertyListing {
  const data = apiData as Record<string, unknown>;
  return {
    id: (data.id ?? data.listingid ?? data.detailreq ?? "") as string,
    reference: (data.reference ?? data.detailreq ?? "") as string,
    title: (data.title ?? "") as string,
    type: (data.type ?? "house") as "house" | "apartment" | "office" | "land" | "townhouse" | "commercial space" | "warehouse" | "guest house" | "shop" | "retail" | "beach house",
    contract: (data.contract ?? "rent") as "rent" | "sale" | "short-stay",
    status: (data.status ?? "active") as "active" | "pending" | "sold" | "rented" | "withdrawn",
    location: (data.location ?? data.streetaddress ?? "") as string,
    bedrooms: parseInt((data.bedrooms ?? data.beds ?? data.bedroomcount ?? "0") as string),
    bathrooms: parseInt((data.bathrooms ?? data.baths ?? data.bathroomcount ?? "0") as string),
    ...
    slug: (data.slug ?? generatePropertySlug(data.title as string, data.type as string, data.id as string)) as string,
    agent: {
      name: ((data.agent as Record<string, unknown>)?.name ?? (data.owner as Record<string, unknown>)?.name ?? "Unknown") as string,
      verified: Boolean(
        (data.agent as Record<string, unknown>)?.verified ?? (data.owner as Record<string, unknown>)?.verification === "1"
      ),
    },
  };
}
 ...
 */
export function transformPropertyListingToLegacy(property: PropertyListing): Record<string, unknown> {
  return {
 ...
 */
export function transformApiToPropertyProject(apiData: unknown): PropertyProject {
  const data = apiData as Record<string, unknown>;
  return {
    id: (data.id ?? data.projectid ?? "") as string,
    name: (data.name ?? data.projectname ?? "") as string,
    slug: (data.slug ?? generateProjectSlug(data.name as string, data.id as string)) as string,
    ...
    developer: {
      ...
      name: (data.developername ?? (data.developer as Record<string, unknown>)?.name ?? "Unknown") as string,
      logo: (data.developer as Record<string, unknown>)?.logo as string | undefined,
    ...
    media: {
      coverImage: (data.coverphoto ?? data.coverimage ?? data.image ?? "") as string,
      images: Array.isArray(data.images) 
        ? data.images as string[]
        : [(data.coverphoto ?? data.image) as string].filter(Boolean),
      tourVideo: (data.tourvideo ?? data.tourVideo) as string | undefined,
    },
    features: Array.isArray(data.features) ? data.features as ("Swimming Pool" | "Gym" | "Garden" | "Parking" | "Security" | "Generator" | "Air Conditioning" | "Furnished" | "Balcony" | "Terrace" | "Elevator" | "CCTV" | "Gated Community" | "Playground" | "24hr Water" | "24hr Electricity")[] : [],
    ...
    units: {
      ...
      types: Array.isArray(data.unittypes) ? (data.unittypes as Record<string, unknown>[]).map((type) => ({
        type: (type.type ?? "apartment") as "house" | "apartment" | "office" | "land" | "townhouse" | "commercial space" | "warehouse" | "guest house" | "shop" | "retail" | "beach house",
        count: parseInt((type.count ?? "0") as string),
        priceFrom: parseFloat((type.priceFrom ?? type.price ?? "0") as string),
      })) : [],
    },
    completion: {
      status: (data.status ?? "under_construction") as "under_construction" | "completed" | "planned",
      date: (data.completiondate ?? data.completionDate) as string | undefined,
    },
    dates: {
      launched: (data.dateadded ?? data.created_at ?? new Date().toISOString()) as string,
 ...
 */
export function transformApiToPropertyUnit(apiData: unknown, projectId?: string): PropertyUnit {
  const data = apiData as Record<string, unknown>;
  return {
    id: (data.unitid ?? data.id ?? "") as string,
    unitNumber: (data.unitnumber ?? data.unitname) as string | undefined,
    type: (data.unittype ?? "apartment") as "house" | "apartment" | "office" | "land" | "townhouse" | "commercial space" | "warehouse" | "guest house" | "shop" | "retail" | "beach house",
    title: (data.title ?? data.unitname ?? `Unit ${String(data.unitnumber ?? data.id)}`) as string,
...
export function transformApiToBlogResponse(apiData: unknown): BlogResponse {
  const data = apiData as Record<string, unknown>;
  return {
    featured: Array.isArray(data.featured) ? (data.featured as unknown[]).map(transformBlogPost) : [],
    market: Array.isArray(data.market) ? (data.market as unknown[]).map(transformBlogPost) : [],
  };
}
...
function transformBlogPost(post: unknown) {
  ...
  return {
    thumbnail: (data.coverImage ?? data.image ?? data.thumbnail ?? "") as string,
    source: "meQasa Blog",
    summary: (data.title ?? data.excerpt ?? data.summary ?? "") as string,
    date: (data.publishedAt ?? data.created_at ?? data.date ?? new Date().toISOString()) as string,
...
export const batchTransformers = {
  ...
  toPropertyUnits(apiDataArray: unknown[], projectId?: string): PropertyUnit[] {
    return (apiDataArray ?? []).map(data => transformApiToPropertyUnit(data, projectId));
  },
};


function generatePropertySlug(title?: string, type?: string, id?: string): string {
  if (!title && !type && !id) return "property";
  
  const parts = [];
  if (title) parts.push(title);
  if (type) parts.push(getPropertyTypeLabel(type as "land" | "house" | "apartment" | "office" | "townhouse" | "commercial space" | "warehouse" | "guest house" | "shop" | "retail" | "beach house"));
  if (id) parts.push(id);
