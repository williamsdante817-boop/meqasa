...
export async function processUnifiedReferenceSearch(
  ...
  try {
    ...
    switch (strategy.strategy) {
      case "primary-first":
        result = await searchPrimaryFirst(
          detection,
          strategy,
          startTime,
          timeout
...
async function searchPrimaryFirst(
  ...
  const primaryType =
    strategy.primaryType === "unknown" || !strategy.primaryType
      ? "property"
  ...
  try {
    ...
    if (primaryResult.isValid) {
      return {
        ...
        source: primaryType,
        responseTime: performance.now() - startTime,
        [primaryType === "property" ? "propertyResult" : "unitResult"]:
          primaryResult,
      };
    }
  } catch (error) {
  ...
  try {
    ...
    if (fallbackResult.isValid) {
      return {
        ...
        source: fallbackType,
        responseTime: performance.now() - startTime,
        [fallbackType === "property" ? "propertyResult" : "unitResult"]:
          fallbackResult,
      };
    }
  } catch (error) {
...
async function searchParallel(
  ...
  try {
    ...
    const [propertyPromise, unitPromise] = [
      searchByType(detection.reference, "property").catch(
        (err) =>
          ({
            reference: detection.reference,
            url: "",
      ...
      searchByType(detection.reference, "unit").catch(
        (err) =>
          ({
            reference: detection.reference,
            url: "",
...
function selectBestResult(
  ...
  UnifiedSearchResult,
  "searchType" | "strategy" | "responseTime" | "propertyResult" | "unitResult"
> {
...
async function findAlternativeResults(
  ...
  Array<{ type: "property" | "unit"; url: string; reference: string }>
> {
  try {
...
function getBestErrorMessage(
  ...
  if (
    propertyResult.error?.includes("Invalid") ||
    unitResult.error?.includes("Invalid")
  ) {
