...
type EstablishmentData = Record<string, Record<Establishment['type'], Array<{
  name: string;
  address: string;
  ...
  'adjiringanor-east-legon': {
    school: [
      {
        ...
        coordinates: { lat: 5.6510, lng: -0.1462 },
        rating: 4.8,
    ...
    bank: [
      ...
      },
      {
        ...
        address: 'East Legon, Accra',
        coordinates: { lat: 5.6490, lng: -0.1440 },
        rating: 4.3,
        phone: '+233302662950',
        openNow: true
      },
      {
        ...
        address: 'East Legon, Accra',
        coordinates: { lat: 5.6520, lng: -0.1470 },
        rating: 4.2,
        phone: '+233302123456',
        openNow: false
      }
    ...
    hospital: [
      {
        ...
        address: 'East Legon, Accra',
        coordinates: { lat: 5.6530, lng: -0.1450 },
        rating: 4.5,
        phone: '+233302501234',
      ...
      },
      {
        ...
        coordinates: { lat: 5.6470, lng: -0.1485 },
        rating: 4.6,
        phone: '+233302761391',
    ...
    supermarket: [
      {
        ...
        address: 'East Legon, Accra',
        coordinates: { lat: 5.6500, lng: -0.1460 },
        rating: 4.4,
  ...
  'adenta': {
    school: [
      {
        ...
        coordinates: { lat: 5.7010, lng: -0.1680 },
        rating: 4.0,
        openNow: true
      },
      {
    ...
    hospital: [
      {
        ...
        address: 'Adenta Municipality, Accra',
        coordinates: { lat: 5.7000, lng: -0.1690 },
        rating: 4.2,
        phone: '+233302456123',
        openNow: true
      }
    ...
    supermarket: [
      ...
      },
      {
        name: 'Melcom Adenta',
        address: 'Adenta, Accra',
        coordinates: { lat: 5.7020, lng: -0.1670 },
        rating: 3.8,
        openNow: true
      }
    ],
...
export async function getEstablishmentsFromGoogleMaps(
  ...
  try {
    ...
    try {
      const geocodeResponse = await fetch(`/api/google-maps/geocode?address=${encodeURIComponent(neighborhood)}`);
      ...
      if (projectLocation && isValidCoordinates(projectLocation)) {
        ...
        throw new Error(`Failed to geocode location "${neighborhood}" and no valid coordinates provided`);
      }
    }
        ...
        if (placesResponse.ok) {
          ...
          if (placesData.success && placesData.places) {
            placesData.places.forEach((place: any, index: number) => {
              ...
              if (filters?.minRating && place.rating && place.rating < filters.minRating) return;
            ...
            console.log(`📍 Found ${placesData.places.length} ${type}(s) from Google Places (${placesData.filtered_count} after server-side filtering from ${placesData.total_found} raw results)`);
          }
        } else {
    ...
    console.log(`📍 Total found ${establishments.length} establishments from Google Maps`);
    ...
    const fallbackLocation = projectLocation && isValidCoordinates(projectLocation) 
      ? projectLocation 
      : { lat: 5.6037, lng: -0.1870 }; // Default Accra coordinates
      
    return getEstablishmentsFromLocalData(fallbackLocation, neighborhood, filters);
  }
}
...
async function getEstablishmentsFromLocalData(
  ...
  Object.entries(locationData).forEach(([type, typeEstablishments]) => {
    ...
    typeEstablishments?.forEach((establishment) => {
      ...
      // Apply filters
      if (filters?.maxDistance && distance > filters.maxDistance) return;
      if (filters?.minRating && establishment.rating && establishment.rating < filters.minRating) return;
      if (filters?.openNow !== undefined && establishment.openNow !== filters.openNow) return;
...
export async function getEstablishments(
  ...
  try {
    ...
    if (apiKey && neighborhood) {
      ...
      console.log('Google Maps API key not found or neighborhood not provided, using local data');
      ...
      return await getEstablishmentsFromLocalData(fallbackLocation, neighborhood, filters);
    }
  } catch (error) {
...
export function getEstablishmentTypeCounts(establishments: Establishment[]): Record<Establishment['type'], number> {
  const counts = {
    school: 0,
    bank: 0,
    hospital: 0,
