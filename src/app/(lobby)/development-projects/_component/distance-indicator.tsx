...
function DistanceIndicatorComponent({
  ...
  if (variant === "compact") {
    return (
      <div className={cn("flex items-center gap-1 text-brand-blue font-medium text-xs", className)}>
        <MapPin className="w-3 h-3" aria-hidden="true" />
        <span>{formattedDistance}{unit !== "m" ? unit : ""}</span>
      </div>
    );
  }
    ...
    return (
      <div className={cn("space-y-1", className)}>
        <div className="flex items-center gap-2 text-brand-blue font-medium text-sm">
          ...
          <span>{formattedDistance}{unit !== "m" ? unit : ""} away</span>
        </div>
  ...
  return (
    <div className={cn(
      ...
      <MapPin className="w-4 h-4 transition-transform group-hover:scale-110" aria-hidden="true" />
