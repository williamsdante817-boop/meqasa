...
const ENHANCED_TYPE_MAPPING: Record<string, { 
  types: string[], 
  keywords?: string[], 
  ...
  school: {
    ...
    keywords: ['school', 'academy', 'college', 'university', 'kindergarten', 'nursery'],
...
function filterRelevantPlaces(places: any[], requestedType: string): any[] {
  ...
  return places.filter(place => {
    ...
    const hasRelevantKeyword = typeConfig.keywords?.some(keyword => 
      placeName.includes(keyword.toLowerCase())
    ) || false;
    
    // Check if place name contains excluded keywords
    const hasExcludedKeyword = typeConfig.excludeKeywords?.some(keyword => 
      placeName.includes(keyword.toLowerCase())
    ) || false;
...
export async function GET(request: NextRequest) {
  ...
  try {
    console.log(`🔍 Server-side places search: ${type} near ${location} (radius: ${radius}m, limit: ${limit})`);
    ...
    console.log(`📝 Raw results: ${allPlaces.length} places found before filtering`);
    ...
    console.log(`✅ Filtered to ${places.length} relevant ${type} establishments (from ${uniquePlaces.length} unique results)`);
